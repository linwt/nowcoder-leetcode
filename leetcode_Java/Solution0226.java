...
=========================================================
新模板注释，递归
1、方法功能：入参是一个节点，将该节点的左右子节点交换，返回该节点
2、终止条件：节点为空时返回空
3、一个节点处理过程和返回结果：将该节点的左右子节点交换，返回该节点
4、递归调用：左右节点的子节点同样需要交换，因此调用同样的方法递归处理
5、递归顺序：前序遍历，先把根节点的子节点交换完，从上往下按序进行交换
6、使用递归调用结果和返回结果：不用接收调用结果，处理交换即可，最后返回根节点
* */
class Solution {
